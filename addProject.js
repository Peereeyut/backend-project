...
router.get("/getforadvisor/:name", (req, res) => {
  var name = req.params.name.substring(1)
  var sql = `SELECT *
  FROM advisor
  WHERE idadvisor LIKE '%${name}%'
     OR ad_en_first_name LIKE '%${name}%'
     OR ad_en_last_name LIKE '%${name}%'
     OR ad_th_first_name LIKE '%${name}%'
     OR ad_th_last_name LIKE '%${name}%'`;
  db.query(sql, function (error, result) {
    if (error) {
      console.log("Error Connecting to DB st1");
    } else {

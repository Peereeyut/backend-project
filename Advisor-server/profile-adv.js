...
router.get("/inviteadvisorall", (req, res) => {
    ...
    var sql = `SELECT * FROM project INNER JOIN inviteadvisor ON project.idProject = inviteadvisor.Project_idProject INNER JOIN advisor ON advisor.idadvisor=inviteadvisor.advisor_idadvisor`;
...
router.get("/advisor/:idadvisor", (req, res) => {
    ...
    db.query(sql, function (error, result) {
        if (error) {
            console.log("Error Connecting to DB Get ADV profile",error);
        } else {
            res.send({ status: true, data: result });
        }
    });
...
router.get("/advisor_phone/:idadvisor", (req, res) => {
    ...
    db.query(sql, function (error, result) {
        if (error) {
            console.log("Error Connecting to DB ADV Phaone profile",error);
        } else {
            res.send({ status: true, data: result });
        }
    });
...
router.get("/advisor_email/:idadvisor", (req, res) => {
    ...
    db.query(sql, function (error, result) {
        if (error) {
            console.log("Error Connecting to DB Get ADV Email profile",error);
        } else {
            res.send({ status: true, data: result });
        }
    });
...
router.get("/project_advisor/:idadvisor", (req, res) => {
    ...
    db.query(sql, function (error, result) {
        if (error) {
            console.log("Error Connecting to DB Get Project ADV profile",error);
        } else {
            res.send({ status: true, data: result });
        }
    });
...
router.get("/chartadvisor/:idadvisor", (req, res) => {
    ...
    WHERE project_advisor.Advisor_idAdvisor = 1
